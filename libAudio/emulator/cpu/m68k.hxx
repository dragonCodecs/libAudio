...
private:
	...
	// What IRQs are pending execution (7 levels, masked by the status register IRQ bits)
	uint8_t pendingIRQs{0U};
	...
	[[nodiscard]] stepResult_t dispatchEOR(const decodedOperation_t &insn) noexcept;
	[[nodiscard]] stepResult_t dispatchEORI(const decodedOperation_t &insn) noexcept;
...
public:
	...
	void writeStatus(uint16_t value) noexcept;
