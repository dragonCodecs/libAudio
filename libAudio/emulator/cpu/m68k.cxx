...
motorola68000_t::motorola68000_t(memoryMap_t<uint32_t, 0x00ffffffU> &peripherals, const uint32_t clockFreq) noexcept :
	...
	switch (insn & insnMaskEANoReg)
	{
		...
		case 0xe2c0U:
			...
			return
			{
				...
				eaReg,
				{operationFlags_t::memoryNotRegister},
				// The EA form of this instruction must be executed in u16 mode
				1U, 0U,
				eaMode,
			};
		case 0xe3c0U:
...
stepResult_t motorola68000_t::step() noexcept
{
	...
	switch (instruction.operation)
	{
		...
		case instruction_t::asr:
			return dispatchASR(instruction);
		...
		case instruction_t::lsl:
			return dispatchLSL(instruction);
		case instruction_t::lsr:
			return dispatchLSR(instruction);
...
stepResult_t motorola68000_t::dispatchAND(const decodedOperation_t &insn) noexcept
{
	// Unpack the operation size to a value in bytes
	const auto operationSize{unpackSize(insn.operationSize)};
	// Figure out the effective address operand as much as possible so we know where to go poking
	const auto effectiveAddress{computeEffectiveAddress(insn.mode, insn.ry, operationSize)};
	// Grab the data register value used as one of the operands (logical and is commutative, so don't
	...
	// With the two values retreived, do the logical and
...
stepResult_t motorola68000_t::dispatchASR(const decodedOperation_t &insn) noexcept
{
	// Figure out the operation width
	const auto operationSize{unpackSize(insn.operationSize)};
	const auto operationBits{operationSize * 8U};
	...
	const auto value{dataRegister(insn.ry) & mask};
	// Extract the value to use as the new carry and extend bits
	const auto carry{shift == 0U ? 0U : (value >> (shift - 1U)) & 1U};
	// Build the new sign bits to use in the shifting
	const auto extensionBits{(value & signBit) ? 0xffffffffffffffffU : 0U};
...
stepResult_t motorola68000_t::dispatchEORI(const decodedOperation_t &insn) noexcept
{
	// Deal with the special register forms first
	switch (insn.ry)
	{
		case 8U:
		case 9U:
			return dispatchEORISpecialCCR(insn);
	}
	// Unpack the operation size to a value in bytes
	const auto operationSize{unpackSize(insn.operationSize)};
	// Extract the immediate that follows this instruction
...
stepResult_t motorola68000_t::dispatchEORISpecialCCR(const decodedOperation_t &insn) noexcept
{
	// If this instruction is for the SR specifically, it is privileged, so check that
	if (insn.ry == 9U && status.excludes(m68kStatusBits_t::supervisor))
		return {true, true, 0U};
	...
	const auto rhs
	{
		[&]()
		{
			if (insn.ry == 9U)
				return static_cast<uint16_t>(readImmediateUnsigned(2U));
			return static_cast<uint16_t>(readImmediateUnsigned(1U));
		}()
	};
	// Grab the SR
	const auto lhs{status.toRaw()};
	// Apply the exclusive or and write it back
	status.fromRaw(lhs ^ rhs);
...
stepResult_t motorola68000_t::dispatchIllegal() noexcept
{
	// Compute where the TRAP handler is (vector 4)
	const auto vectorAddress{static_cast<uint16_t>(4U << 2U)};
	// Grab the old status register value
	const auto statusReg{status.toRaw()};
	// Force supervisor mode
	status.set(m68kStatusBits_t::supervisor);
...
stepResult_t motorola68000_t::dispatchLSL(const decodedOperation_t &insn) noexcept
{
	// Figure out the operation width
	const auto operationSize{unpackSize(insn.operationSize)};
	const auto operationBits{operationSize * 8U};
	...
	// LSL has 3 forms, check if this is the effective addressed form
	if (insn.flags.includes(operationFlags_t::memoryNotRegister))
	{
		// Extract the address of the manipulation target
		const auto effectiveAddress{computeEffectiveAddress(insn.mode, insn.ry, operationSize)};
...
stepResult_t motorola68000_t::dispatchLSR(const decodedOperation_t &insn) noexcept
{
	// Figure out the operation width
	const auto operationSize{unpackSize(insn.operationSize)};
	// Compute which bit is the sign bit of the result
...
stepResult_t motorola68000_t::dispatchMULS(const decodedOperation_t &insn) noexcept
{
	// This form of MULS is required to be in s16 mode, so grab the target of the EA as the LHS
	const auto lhs{readEffectiveAddress<int16_t>(insn.mode, insn.ry)};
	// And then the RHS from the destination register, truncated to a s16
	const auto rhs{static_cast<int16_t>(dataRegister(insn.rx))};
	// Multiply the two together to make a s32 result
	const auto result{int32_t{lhs} * int32_t{rhs}};
	...
	dataRegister(insn.rx) = static_cast<uint32_t>(result);
	return {true, false, 0U};
}
