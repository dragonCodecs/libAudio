...
private:
	...
	void testAND()
	{
		writeAddress(0x000000U, uint16_t{0xc080U}); // and.l d0, d0
		writeAddress(0x000002U, uint16_t{0xc050U}); // and.w (a0), d0
		writeAddress(0x000004U, uint16_t{0xc03cU});
		writeAddress(0x000006U, uint16_t{0x0060U}); // and.b #$60, d0
		writeAddress(0x000008U, uint16_t{0xc199U}); // and.l d0, (a1)+
		writeAddress(0x00000aU, uint16_t{0xc07cU});
		writeAddress(0x00000cU, uint16_t{0x0000U}); // and.w #$00, d0
		writeAddress(0x00000eU, uint16_t{0x4e75U}); // rts to end the test
		writeAddress(0x000100U, uint32_t{0x4400ffffU});
		// Set the CPU to execute this sequence
		cpu.executeFrom(0x00000000U, 0x00800000U);
		...
		// Make sure d0 starts out with a sensible value for this sequence
		cpu.writeDataRegister(0U, 0x7f00ffffU);
		...
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000002U);
		assertEqual(cpu.readDataRegister(0U), 0x7f00ffffU);
		assertEqual(cpu.readStatus(), 0x0010U);
		// Step the second instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000004U);
		assertEqual(cpu.readDataRegister(0U), 0x7f00c03cU);
		assertEqual(cpu.readStatus(), 0x0018U);
		// Step the third instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000008U);
		assertEqual(cpu.readDataRegister(0U), 0x7f00c020U);
		assertEqual(cpu.readStatus(), 0x0010U);
		// Step the fourth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000000aU);
		...
		assertEqual(readAddress<uint32_t>(0x000100U), uint32_t{0x4400c020U});
		assertEqual(cpu.readStatus(), 0x0010U);
		// Step the fourth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000000eU);
		assertEqual(cpu.readDataRegister(0U), 0x7f000000U);
	...
	void testBcc()
	{
		...
		// Step the nineth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000001aU);
		// Set up the status register to not skip the next jump
		cpu.writeStatus(0x001bU);
		// Step the tenth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000001eU);
		// Step the eleventh instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000020U);
		// Set up the status register to not skip the next jump
		cpu.writeStatus(0x001fU);
		// Step the twelveth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000024U);
		// Step the thirteenth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000026U);
		// Set up the status register to not skip the next jump
		cpu.writeStatus(0x001dU);
		// Step the fourteenth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000002aU);
		// Step the fiftheenth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000002cU);
		// Set up the status register to not skip the next jump
		cpu.writeStatus(0x001fU);
		...
		// Set up the status register to not skip the next jump
		cpu.writeStatus(0x001fU);
		// Step the twenthieth instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000003cU);
		// Set up the status register to skip the next jump
		cpu.writeStatus(0x0017U);
		// Step the twentyfirst instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000003eU);
		// Set up the status register to not skip the next jump
		cpu.writeStatus(0x001fU);
		// Step the twentysecond instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000042U);
		// Step the twentythird instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000044U);
		// Set up the status register to not skip the next jump
		cpu.writeStatus(0x0017U);
	...
	void testDBcc()
	{
		writeAddress(0x000000U, uint16_t{0x51c8U});
		writeAddress(0x000002U, uint16_t{0xfffeU}); // dbf d0, #-2
		writeAddress(0x000004U, uint16_t{0x5dc9U});
		writeAddress(0x000006U, uint16_t{0xfffcU}); // dblt d1, #-4
		writeAddress(0x000008U, uint16_t{0x50c9U});
		writeAddress(0x00000aU, uint16_t{0xfff8U}); // dbt d1, #-8
		writeAddress(0x00000cU, uint16_t{0x4e75U}); // rts to end the test
		// Set the CPU to execute this sequence
		cpu.executeFrom(0x00000000U, 0x00800000U);
		// Set up d0, and d1 to sensible values
		cpu.writeDataRegister(0U, 0x08080001U);
		cpu.writeDataRegister(1U, 0x00068000U);
		// Set the status register to the a valid less-than condition configuration
		cpu.writeStatus(0x0008U);
		// Validate starting conditions
		assertEqual(cpu.readProgramCounter(), 0x00000000U);
		assertEqual(cpu.readAddrRegister(7U), 0x007ffffcU);
		// Step the first instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000000U);
		assertEqual(cpu.readDataRegister(0U), 0x08080000U);
		assertEqual(cpu.readStatus(), 0x0008U);
		// Step the second instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000004U);
		assertEqual(cpu.readDataRegister(0U), 0x0808ffffU);
		assertEqual(cpu.readStatus(), 0x0008U);
		// Step the third instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000008U);
	...
	void testEORI()
	{
		writeAddress(0x000000U, uint16_t{0x0a38U});
		writeAddress(0x000002U, uint16_t{0x0024U});
		writeAddress(0x000004U, uint16_t{0x0100U}); // eori.b #$24, $0100.w
		writeAddress(0x000006U, uint16_t{0x0a40U});
		writeAddress(0x000008U, uint16_t{0x0000U}); // eori.w #0, d0
		writeAddress(0x00000aU, uint16_t{0x0a80U});
		writeAddress(0x00000cU, uint32_t{0xc0000001U}); // eori.l #$c0000001, d0
		writeAddress(0x000010U, uint16_t{0x4e75U}); // rts to end the test
		writeAddress(0x000100U, uint8_t{0x21U});
		// Set the CPU to execute this sequence
		cpu.executeFrom(0x00000000U, 0x00800000U);
		// Set up d0 so we get zero = true from the second instruction, but get something interesting for the 3rd
		cpu.writeDataRegister(0U, 0x45a00000U);
		// Set the carry and overflow bits in the status register so we can observe them being cleared
		// And the extend bit to make sure that's left alone
		cpu.writeStatus(0x0013U);
		// Validate starting conditions
		...
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000006U);
		assertEqual(readAddress<uint8_t>(0x000100U), uint8_t{0x05U});
		assertEqual(cpu.readStatus(), 0x0010U);
		// Step the second instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000000aU);
		assertEqual(cpu.readDataRegister(0U), 0x45a00000U);
		assertEqual(cpu.readStatus(), 0x0014U);
		// Step the third instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000010U);
	...
	void testEORISpecial()
	{
		writeAddress(0x000000U, uint16_t{0x0a3cU});
		writeAddress(0x000002U, uint16_t{0x000aU}); // eori #$0a, ccr
		writeAddress(0x000004U, uint16_t{0x0a3cU});
		writeAddress(0x000006U, uint16_t{0xff15U}); // eori #$15, ccr w/ intentional value misencode
		writeAddress(0x000008U, uint16_t{0x0a7cU});
		writeAddress(0x00000aU, uint16_t{0x070eU}); // eori #$070e, sr
		writeAddress(0x00000cU, uint16_t{0x4e75U}); // rts to end the test
		// Set the CPU to execute this sequence
		cpu.executeFrom(0x00000000U, 0x00800000U);
		// Validate starting conditions
		...
		cpu.writeStatus(0x2700U);
		// Step the third instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000000cU);
		assertEqual(cpu.readStatus(), 0x200eU);
		...
		// This sequence should trap
		writeAddress(0x000000U, uint16_t{0x0a7cU});
		writeAddress(0x000002U, uint16_t{0x2705U}); // eori #$2705, sr
		// Set the CPU to execute this sequence
		cpu.executeFrom(0x00000000U, 0x00800000U);
		// Validate starting conditions
		assertEqual(cpu.readProgramCounter(), 0x00000000U);
	...
	void testORI()
	{
		...
		writeAddress(0x00000cU, uint32_t{0x80000001U}); // ori.l #$80000001, d0
		writeAddress(0x000010U, uint16_t{0x4e75U}); // rts to end the test
	...
	void testSUBI()
	{
		writeAddress(0x000000U, uint16_t{0x0418U});
		writeAddress(0x000002U, uint16_t{0x0004U}); // subi.b #4, (a0)+
		writeAddress(0x000004U, uint16_t{0x0458U});
		writeAddress(0x000006U, uint16_t{0x8001U}); // subi.w #$8001, (a0)+
		writeAddress(0x000008U, uint16_t{0x0498U});
		writeAddress(0x00000aU, uint32_t{0x7fffffffU}); // subi.l #$7fffffff, (a0)+
		writeAddress(0x00000eU, uint16_t{0x0428U});
		writeAddress(0x000010U, uint16_t{0x003cU});
		writeAddress(0x000012U, uint16_t{0xfff9U}); // subi.b #$3c, -7(a0)
		writeAddress(0x000014U, uint16_t{0x4e75U}); // rts to end the test
		writeAddress(0x000100U, uint8_t{0x40U});
		writeAddress(0x000101U, uint16_t{0x7ffeU});
		writeAddress(0x000103U, uint32_t{0xfeed4ca7U});
		// Set the CPU to execute this sequence
		cpu.executeFrom(0x00000000U, 0x00800000U);
		// Set up a0 to point to the data pool at +0x0100U
		cpu.writeAddrRegister(0U, 0x00000100U);
		...
		assertEqual(cpu.readProgramCounter(), 0x00000004U);
		assertEqual(cpu.readAddrRegister(0U), 0x00000101U);
		assertEqual(readAddress<uint8_t>(0x00000100U), 0x3cU);
		assertEqual(cpu.readStatus(), 0x0000U);
		// Step the second instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x00000008U);
		assertEqual(cpu.readAddrRegister(0U), 0x00000103U);
		assertEqual(readAddress<uint16_t>(0x00000101U), 0xfffdU);
		assertEqual(cpu.readStatus(), 0x001bU);
		// Step the third instruction and validate
		runStep();
		assertEqual(cpu.readProgramCounter(), 0x0000000eU);
